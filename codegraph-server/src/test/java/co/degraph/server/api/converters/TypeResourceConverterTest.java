...
import co.degraph.server.api.resources.FieldResource;
import co.degraph.server.api.resources.MethodResource;
import co.degraph.server.api.resources.ParameterResource;
import co.degraph.server.api.resources.TypeResource;
...
public class TypeResourceConverterTest {
    ...
    public void shouldConvertATypeResourceToModel() {
        ...
        type.setPackageName("co.degraph.server.api.converters");
        ...
        FieldResource fieldResource = new FieldResource("converter", type("TypeResourceConverter", "co.degraph.server.api.converters"));
        type.setFields(asList(fieldResource));

        Type node = converter.toModel(type);

        assertThat(node.getName(), is("TypeResourceConverterTest"));
        assertThat(node.getPackageName(), is("co.degraph.server.api.converters"));
    ...
    public void shouldConvertAModelTypeToResource() {
        ...
        Type type = new Type("TypeResourceConverterTest", "co.degraph.server.api.converters", "application", "class");
        type.addMethod(new Method("shouldConvertAModelTypeToResource", asList(new Parameter(1, list)), asList(list)));
        type.addField(new Field("converter", new Type("TypeResourceConverter", "co.degraph.server.api.converters", "application", "class")));
        ...
        assertThat(resource.getPackageName(), is("co.degraph.server.api.converters"));
